function appViewModel() {
    ...
    /**
     * Creates the map and sets the center to Tampa.  Then gets popular
     * restaurants and cafes in the area.
     */
    function initialize() {
        tampa = new google.maps.LatLng(27.951697, -82.453511);
    ...
    /**
     * Makes a request to Google for popular restaurants and bars in tampa.
    ...
    function getInstagrams(place) {
        ...
        var accessToken = '1920021740.5b9e1e6.c5328fb8bd5949bcbda8f181a9f89fbc';
    ...
    // the getAllPlaces function - An array with all places initially retrieved
    ...
    self.displayInfo = function (place) {
        ...
        service.getDetails(request, function (details, status) {
            // values to display if getDetails fails.
    ...
    // Display the photo at index of chosenPlace's instagram array.
    ...
    self.getInstagramStatus = ko.computed(function () {
        if (self.chosenPlace()) {
            if (self.chosenPlace().instagrams().length != 0 &&
                ...
                return 'Click to view recent Instagrams for' +
            ...
            if (self.chosenPlace().instagrams().length === 0 &&
                !self.chosenPlace().isGettingInstagrams()) {
                return 'No Instagrams found for' + self.chosenPlace().name;
            }
        }
        return '';
    });
    ...
    // Use the left and right arrow keys to go through photo gallery.
    ...
    // stop the marker's movement and deselect the place as chosenPlace.
    google.maps.event.addListener(infowindow,'closeclick',function(){
        self.chosenPlace().marker.setAnimation(null);
        self.chosenPlace(null);
    });

    // if the width is less than 650px, hide the list view
    $(function () {
        if ($(window).width() < 650) {
            if (self.displayingList()) {
                self.displayingList(false);
